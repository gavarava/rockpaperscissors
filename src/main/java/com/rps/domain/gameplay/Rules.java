...
public class Rules {
  ...
  public ActionType evaluate(ActionType actionType1, ActionType actionType2) {
    ...
    } else if ((isRock(actionType1) && isPaper(actionType2)) || (isPaper(actionType1) && isRock(
        actionType2))) {
      return ActionType.PAPER;
    } else if ((isRock(actionType1) && isScissors(actionType2)) || (isScissors(actionType1)
        && isRock(
        actionType2))) {
      return ActionType.ROCK;
    } else if ((isPaper(actionType1) && isScissors(actionType2)) || (isScissors(actionType1)
        && isPaper(
        actionType2))) {
